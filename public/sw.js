...
function resolveNotificationDestination(type, refId, questionId) {
  if (typeof type !== "string") return null

  const normalized = type.toLowerCase()
  const id = toPositiveSafeInteger(refId)
  ...
  if (normalized.includes("question") || normalized.includes("answer")) {
    const targetId = id ?? toPositiveSafeInteger(questionId)
    return targetId === null ? null : `/questions/detail/?questionId=${targetId}`
  }
...
function notificationFromPayload(payload) {
  ...
  if (payload.kind === "notification") {
    ...
    const notificationId = Number.isSafeInteger(payload.notificationId) && payload.notificationId > 0
      ...
      : null
    // type/refId 로 딥링크를 계산하고, 새 질문 푸시는 questionId 를 보조 식별자로 쓴다.
    // 매핑 불가하면 알림센터로 폴백한다.
