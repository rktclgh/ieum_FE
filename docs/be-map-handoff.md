...
- [ ] 아래 canonical 목록의 root + 26개 구현 route 전체를 Spring forward/controller/JAR smoke에서 검증
...
이 목록은 FE `scripts/ci/verify-static-export.sh`의 root + 26개 구현 route와 정확히 같아야 한다. Spring controller test와 실제 JAR smoke는 일부 대표 route만 검사하지 않고 아래 전체를 고정한다. query string은 forward 대상을 바꾸지 않으며 browser URL에 보존한다.
...
- 실제 JAR로 root + 26개 구현 route의 HTML과 `index.txt`를 모두 smoke test한다.
