...
Leaflet `Marker eventHandlers`(click) 대신 레이어별 클릭 리스너로 교체:
...
- 내 위치 마커는 지금처럼 클릭을 통과시킨다(`interactive: false`와 동등하게, `marker-icon` 레이어
  hit-test 대상에서 제외).
...
- **클릭 히트박스 차이**: Leaflet `divIcon`은 사각형 히트박스에 `className: "map-circle-marker"`로
  원 밖 클릭을 걸러냈다(#302). MapLibre `symbol` layer의 히트박스는 아이콘 이미지의 alpha 채널
  또는 bounding box 기준이라 동일 동작을 위해 아이콘 이미지 자체를 원형으로 합성(투명 배경)해야
  한다 — 이미 위 아이콘 합성 설계에 포함됨.
