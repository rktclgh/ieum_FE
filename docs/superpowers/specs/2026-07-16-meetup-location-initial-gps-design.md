...
6. 첫 terminal 결과가 error인 fallback 뒤 watchPosition이 늦게 success를 보내도, 사용자가 GPS 버튼을 누르기 전에는 그 좌표를 viewport·직접입력 대상으로 자동 채택하지 않는다. 지도 클릭은 사용자가 고른 좌표를 기존처럼 우선한다.
...
useGeolocation은 최신 watcher 상태(status)와 별도로 첫 terminal 결과를 한 번만 기록하는 initialStatus를 제공한다. MeetupLocationPicker는 position과 initialStatus를 MeetupLocationMap에 함께 전달한다. 새 순수 resolver가 다음 규칙으로 최초 중심을 결정한다.
...
| initialStatus | position | 지도 표면 | 최초 center | 자동 이동 |
| --- | --- | --- | --- | --- |
| loading | null | MapLoadingSkeleton | 없음 | 없음 |
| success | GPS 좌표 | MapCanvas | GPS 좌표 | 없음 |
| error | null | MapCanvas + fallback 안내 | 서울시청 | 없음 |
| error 후 늦은 success | GPS 좌표 | 이미 fallback MapCanvas | 기존 fallback 유지 | 없음; 위치 버튼을 누르기 전에는 직접입력 target도 비워 둠 |

마지막 행은 geolocation watcher가 timeout 뒤 뒤늦게 성공할 수 있는 경우를 의도적으로 안정화한다. 자동 이동을 다시 만들지 않고, live location marker는 최신 위치로 갱신된다. 사용자는 위치 버튼으로 현재 위치로 이동해 그 좌표를 채택하거나, 지도에서 고른 좌표를 바로 사용할 수 있다.
...
geolocation의 단일 소유자다. position뿐 아니라 immutable initialStatus도 map step에 전달한다. 검색 step은 기존처럼 position만 near 값으로 사용한다.

### useGeolocation

기존 status는 watcher의 최신 결과를 계속 노출한다. initialStatus는 loading에서 첫 success 또는 error로만 전이하고 이후에는 바뀌지 않는다. 이 상태는 장소 선택 초기 viewport 정책에만 사용해, error fallback 뒤 늦은 success가 보이지 않는 GPS 좌표를 자동 선택하는 일을 막는다.

### MeetupLocationMap

resolver 결과가 null일 때는 지도 엔진을 렌더하지 않고 기존 MapLoadingSkeleton을 배경으로 사용한다. 결과가 있으면 recenterTarget ?? initialMapCenter를 MapCanvas에 전달한다. 초기 GPS effect를 제거해 첫 위치 수신이 recenter를 유발하지 않게 한다. fallback lock은 initialStatus와 사용자 GPS 버튼 이벤트에서만 바뀌는 hasExplicitRecenter state로 계산해, render 중 ref mutation이나 effect의 동기 setState를 쓰지 않는다.
...
2. Modify: src/features/map/hooks/use-geolocation.ts — 첫 terminal initialStatus 추가
3. Modify: src/features/meetup/components/meetup-location-picker.tsx — initialStatus 전달
4. Modify: src/features/meetup/components/meetup-location-map.tsx — gate, 초기 auto-recenter 제거, fallback lock, 안내, disabled 전달
5. Modify: src/features/meetup/components/location-list-item.tsx — 재사용 가능한 disabled 버튼 지원
6. Modify: src/lib/i18n/messages/ko.ts 및 6개 locale — fallback 안내 키
7. Create: scripts/ci/test-initial-map-center.ts — 순수 상태 테스트
8. Create: scripts/ci/test-map-source-contracts.mjs — initialStatus 고정, 화면 gate, 초기 auto-recenter 방지 계약
9. Create: scripts/ci/test-map-contracts.sh 및 Modify: scripts/ci/test-client-contracts.sh — 기존 CI bundle에 연결
...
3. source contract로 첫 terminal initialStatus가 고정되고 picker가 이를 전달하며, map이 resolver 결과가 있을 때만 MapCanvas를 마운트하고 fallback 뒤 late GPS를 명시적 recenter 전까지 입력 대상으로 쓰지 않는지 고정한다.
