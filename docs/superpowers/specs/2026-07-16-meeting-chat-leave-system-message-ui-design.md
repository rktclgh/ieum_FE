...
- 강퇴 대상이 방을 열고 있는 경우: 기존 `/user/queue/rooms`의 `remove` event를 `useChatRoomSocket`도 구독한다. 같은 `roomId`의 remove를 받으면 모든 로드된 type별 채팅 목록 캐시에서 방을 먼저 제거하고, group이면 meeting detail/participants 캐시도 제거한다. 이어서 room/messages cache와 side panel을 정리한 뒤 `router.replace(routes.chats())`한다. 자진 나가기 mutation이 pending이어도 서버 remove는 접근권한 제거의 확정 신호이므로 같은 정리·이동을 수행한다. 이 경로는 메시지 topic만으로 접근권한을 추정하지 않는다.
