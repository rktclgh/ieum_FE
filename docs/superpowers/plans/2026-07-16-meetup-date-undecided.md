...
| `src/features/meetup/components/meetup-date-picker.tsx` | 날짜 미정 체크 UI와 휠 비활성화·확정 콜백을 맡는다. |
...
| `src/features/meetup/api/meetup-types.ts` | 서버 계약에 맞춘 optional `schedule`/nullable `firstScheduleId` 타입을 제공한다. |
...
Run: `bash scripts/ci/test-meetup-contracts.sh`  
Expected: TypeScript cannot resolve `create-meetup-schedule` before the helper exists.
...
Run: `bash scripts/ci/test-meetup-contracts.sh`  
Expected: three named tests pass with no TypeScript errors.
...
Add a compile-time usage in the test file that permits `const request: CreateMeetingRequest = { ..., type: "one_time", location, maxMembers: 99 }` without `schedule`, and rejects neither a nullable response `firstScheduleId` nor the date-undecided request shape.

- [ ] **Step 2: Run test to verify it fails**

Run: `bash scripts/ci/test-meetup-contracts.sh`  
Expected: `CreateMeetingRequest` requires `schedule` before the API type change.
...
Change the request response types exactly as follows:
...
Run: `bash scripts/ci/test-meetup-contracts.sh && pnpm typecheck`  
Expected: contract tests pass and TypeScript reports no type errors.
...
Run: `node --test scripts/ci/test-meetup-source-contracts.mjs`  
Expected: missing `disabled`, conditional `schedule`, and checkbox assertions fail.
...
Wrap the existing wheel in `cn(isDateUndecided && "pointer-events-none opacity-40")`; on confirm return `{ date: isDateUndecided ? null : selectedDate, isDateUndecided }`.
...
Run: `pnpm test:contracts && pnpm lint && pnpm typecheck`  
Expected: all client contracts, ESLint, and TypeScript pass without output errors.
...
Run: `pnpm verify`  
Expected: contracts, lint, typecheck, production build, and `verify:out` all pass. If Turbopack is sandbox-blocked from binding a port, rerun the exact build command with approved unrestricted sandbox and record that as environment setup, not a product failure.

- [ ] **Step 4: Inspect final diff**

Run: `git diff origin/develop...HEAD --check && git status --short`  
Expected: no whitespace errors and only intended #173 tracked files.
