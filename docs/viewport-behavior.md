...
- **1차(#411까지)**: `statusBarStyle: default`. 뷰포트 안정성을 택하고 상단 edge-to-edge를 포기했다.
  당시 결론: `black-translucent`는 탭바가 62px 움직이고 홈 지도에 하단 죽은 띠가 상시 남아 비호환.
- **최종(#419)**: `statusBarStyle: black-translucent`. **아래 "후속 측정"에서 그 두 함정을 실기기로
  풀었다.** 흔들림은 하단 요소를 `100lvh` floor에 앵커해서, 죽은 띠는 전면 화면을 `height:100lvh`로
  채워서 없앴다. 상단까지 지도가 깔린다. 대가는 상태바 글자가 전역 흰색이 되는 것(밝은 화면 가독성
  저하)이며 의도된 트레이드오프로 수용했다.

사파리 탭에서는 `safe-area-top`이 실측값으로 들어오고, `100lvh` 앵커는 틀리므로(브라우저 크롬이 lvh를
흔든다) `data-standalone`로 분기해 standalone에서만 앵커를 적용한다.
...
## stuck 뷰포트 확장 강제 (필수 — 이게 없으면 하단 공백)

`height: 100lvh`만으로는 부족하다. **iOS standalone은 로드 시 작은 뷰포트(812)로 시작해 첫 스크롤에 큰 뷰포트(874)로 확장**되고 유지된다. 홈 지도는 문서 스크롤이 없어 812에 **갇히고(stuck)**, 그 상태에서는 하단 62px이 애초에 그려지지 않아 `100lvh` 콘텐츠도 812까지만 보인다 → 하단 흰 공백.

실기기 프로브로 두 상태를 분리 측정해 확인:

| 상태 | innerHeight | 100lvh | 지도 바닥(100lvh 끝) 위치 |
|---|---|---|---|
| 로드 직후(stuck) | 812 | 874 | 812 라인 — 아래 62px 흰 공백 |
| 확장 후 | 874 | 874 | 물리 바닥에 붙음 — 공백 없음 |

**해법(검증됨)**: `body`를 뷰포트보다 1px 높게 둬(`min-height: calc(100lvh + 1px)`, standalone에서만) 항상 스크롤 여지를 만들고, 로드 직후 `window.scrollTo(0, 100)` 넛지(`StandaloneViewportExpander`)로 확장을 사용자 조작 없이 강제한다. 확장 뒤 남는 1px 여지가 재접힘을 막는다. 프로브에서 로드하자마자 innerHeight=874, 지도 바닥이 물리 바닥에 붙는 것을 확인했다.
