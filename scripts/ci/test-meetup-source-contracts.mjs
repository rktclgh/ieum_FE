...
test("새 모임 제출은 이미지 업로드 전부터 동기 래치로 중복 요청을 막고 완료 뒤 해제한다", () => {
  const normalized = screen.replace(/\s+/g, "")

  assert.match(normalized, /constsubmissionLatch=React\.useRef\(false\)/)
  assert.match(normalized, /const\[submissionLocked,setSubmissionLocked\]=React\.useState\(false\)/)
  assert.match(normalized, /constsubmitting=submissionLocked\|\|createMeeting\.isPending/)
  assert.match(
    normalized,
    /consthandleSubmit=async\(\)=>\{if\(!form\.canSubmit\|\|submissionLatch\.current\)return;?if\(!form\.place\)return;?submissionLatch\.current=true;?setSubmissionLocked\(true\);?/
  )
  assert.match(
    normalized,
    /finally\{submissionLatch\.current=false;?setSubmissionLocked\(false\);?\}/
  )
  assert.match(normalized, /disabled=\{!form\.canSubmit\|\|submitting\}/)
})
