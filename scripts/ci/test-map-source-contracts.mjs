...
test("geolocation은 권한 거부만 최초 fallback으로 확정하고 일시 오류는 계속 대기한다", () => {
  ...
  // #339: 캐시된 최근 좌표가 있으면 "success"로, 없으면 "loading"으로 출발한다.
  // 어느 경로에서도 "error"로 출발하지 않는 것이 이 계약의 핵심이다.
  assert.match(
    source,
    /const \[initialStatus, setInitialStatus\] = React\.useState<GeolocationStatus>\(\s*seededPosition \? "success" : "loading"\s*\)/
  )
...
test("#339 홈 지도는 GPS·청크·타일을 직렬로 기다리지 않는다", () => {
  const hook = read("src/features/map/hooks/use-geolocation.ts")
  const home = read("src/features/map/components/home-map-screen.tsx")
  const canvas = read("src/features/map/components/map-canvas.tsx")
  const tileLayer = read("src/features/map/components/vector-tile-layer.tsx")
  ...
  assert.ok(maximumAge, "GEOLOCATION_OPTIONS에 maximumAge가 있어야 한다")
  assert.ok(
    Number(maximumAge[1].replaceAll("_", "")) >= 10_000,
    `maximumAge는 10초 이상이어야 한다 (현재 ${maximumAge[1]})`
  )

  // (3) 지도 청크를 측위 대기와 병렬로 받는다 — dynamic()만 두면 GPS 확보 뒤에야 받기 시작한다.
