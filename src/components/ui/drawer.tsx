...
function DrawerPopup({ className, children, ...props }: DrawerPrimitive.Popup.Props) {
  return (
    <DrawerPrimitive.Popup
      ...
      className={cn(
        "flex w-full max-h-[85vh] flex-col items-center gap-4 rounded-t-3xl bg-white px-4 pt-4 pb-[calc(1rem+var(--safe-area-bottom)+var(--drawer-keyboard-inset,0px))] outline-none [transform:translateY(var(--drawer-swipe-movement-y))] transition-transform duration-base ease-base data-ending-style:translate-y-full data-starting-style:translate-y-full",
        className
      )}
      {...props}
    >
