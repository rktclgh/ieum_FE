...
test("iOS 강제 스크롤 직후 offsetTop이 늦어도 문서 스크롤 기준으로 과보정하지 않는다", () => {
  // 포커스 직후에는 window.scrollY가 먼저 376으로 바뀌고 visualViewport.offsetTop 이벤트가
  // 다음 프레임에 도착할 수 있다. 이 순간 0을 믿으면 438px을 더해 시트가 화면 상단으로 튄다.
  assert.equal(
    resolveSheetKeyboardInset({
      ...MEASURED,
      visualOffsetTop: 0,
